...
describe('POST /prove', () => {
  test('returns 200 with proof on success', async () => {
    ...
    const res = await postJson(VALID_REQUEST);
    const body = (await res.json()) as JsonBody;
  ...
  test('returns 400 for invalid method', async () => {
    const res = await postJson({ ...VALID_REQUEST, method: 'DELETE' });
    const body = (await res.json()) as JsonBody;
  ...
  test('returns 400 for missing responseMatches', async () => {
    const res = await postJson({ url: 'https://api.example.com/price', method: 'GET' });
    const body = (await res.json()) as JsonBody;
  ...
  test('returns 400 for empty responseMatches', async () => {
    const res = await postJson({ ...VALID_REQUEST, responseMatches: [] });
    const body = (await res.json()) as JsonBody;
  ...
  test('returns 400 for private/internal URLs', async () => {
    const res = await postJson({ ...VALID_REQUEST, url: 'http://169.254.169.254/latest/meta-data/' });
    const body = (await res.json()) as JsonBody;
  ...
  test('returns 400 for localhost URLs', async () => {
    const res = await postJson({ ...VALID_REQUEST, url: 'http://localhost:8001/ws' });
    const body = (await res.json()) as JsonBody;
  ...
  test('returns 400 for file:// scheme URLs', async () => {
    const res = await postJson({ ...VALID_REQUEST, url: 'file:///etc/passwd' });
    const body = (await res.json()) as JsonBody;
